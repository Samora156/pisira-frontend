...
export default function Orders() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader title="Order Servis" description={`${orders.length} order ditemukan`}
        action={<Button onClick={() => setAddOpen(true)}><Plus className="h-4 w-4" />Order Baru</Button>} />
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-2">
            <FormField label="Customer" required>
              <Select value={form.customer_id} onValueChange={v => setForm(f => ({ ...f, customer_id: v }))}>
                ...
                <SelectContent>
                  {customers.map(c => <SelectItem key={c.id} value={String(c.id)}>{c.nama} — {c.no_hp}</SelectItem>)}
                </SelectContent>
              </Select>
            </FormField>
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Merk" required>
                <Input placeholder="ASUS, Lenovo, HP..." value={form.merk_laptop}
                  onChange={e => setForm(f => ({ ...f, merk_laptop: e.target.value }))} />
              </FormField>
              <FormField label="Model" required>
                <Input placeholder="VivoBook 14..." value={form.model_laptop}
            ...
            <FormField label="Serial Number" hint="Opsional">
              <Input className="font-mono text-sm" placeholder="SN/IMEI..."
                value={form.sn_laptop} onChange={e => setForm(f => ({ ...f, sn_laptop: e.target.value }))} />
            </FormField>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdd} disabled={saving}>
              {saving && <Spinner className="h-4 w-4" />}{saving ? 'Menyimpan...' : 'Simpan Order'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <div className="space-y-4 py-2">
            ...
            <FormField label="Hasil Diagnosa">
              <Textarea placeholder="Temuan setelah diperiksa..." className="h-20"
                value={statusForm.diagnosa} onChange={e => setStatusForm(f => ({ ...f, diagnosa: e.target.value }))} />
            </FormField>
